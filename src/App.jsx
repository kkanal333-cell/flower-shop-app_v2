...
import { supabase, PURCHASE_PAYMENT_OPTIONS, getKoreaNowFormatted, RIBBON_FONT_OPTIONS, RIBBON_GOOGLE_FONTS_URL } from './shared.js';
import PurchaseTab from './PurchaseTab.jsx';
import StatsTab from './StatsTab.jsx';
import RibbonEditor from './RibbonEditor.jsx';
...
export default function App() {
  ...
  const [orderInfoModal, setOrderInfoModal] = useState(null); // 알림·매출 리스트 클릭 시 주문 정보 요약 팝업 (출력 아님, 조회 전용) - 주문 객체 또는 null
  ...
  const [photoMap, setPhotoMap] = useState({}); // { [order_id]: photo[] } - 주문당 최대 MAX_ORDER_PHOTOS장
  const [photoViewer, setPhotoViewer] = useState(null); // 뷰어에 열려있는 주문의 id (또는 null)
  const [photoViewerIndex, setPhotoViewerIndex] = useState(0); // 뷰어에서 현재 보고 있는 사진 인덱스
  const [photoUploadingOrderId, setPhotoUploadingOrderId] = useState(null);

  const viewerPhotos = photoViewer ? (photoMap[String(photoViewer)] || []) : [];
  const viewerPhoto = viewerPhotos[photoViewerIndex] || null;

  // 뷰어가 열려있는 주문의 사진이 삭제되어 목록이 바뀌면 인덱스를 범위 안으로 보정하고,
  // 사진이 하나도 남지 않으면 뷰어를 자동으로 닫습니다.
  useEffect(() => {
    if (!photoViewer) return;
    if (viewerPhotos.length === 0) {
      setPhotoViewer(null);
      setPhotoViewerIndex(0);
    } else if (photoViewerIndex > viewerPhotos.length - 1) {
      setPhotoViewerIndex(viewerPhotos.length - 1);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [photoMap, photoViewer]);

  // 저장된 모든 작품 사진의 용량 합계 (order_photos 테이블의 size_bytes 합산, 실시간 photoMap 기준)
  const photoStorageUsedBytes = useMemo(
    () => Object.values(photoMap).reduce((sum, arr) => sum + (arr || []).reduce((s, p) => s + (Number(p?.size_bytes) || 0), 0), 0),
    [photoMap]
  );
  const photoTotalCount = useMemo(
    () => Object.values(photoMap).reduce((sum, arr) => sum + (arr ? arr.length : 0), 0),
    [photoMap]
  );
  const photoStorageUsedMB = photoStorageUsedBytes / (1024 * 1024);
  const photoStorageUsedPercent = Math.min(100, (photoStorageUsedMB / SUPABASE_STORAGE_LIMIT_MB) * 100);
  ...
  const loadOrderPhotos = async () => {
    ...
    const next = {};
    (data || []).forEach(photo => {
      const key = String(photo.order_id);
  ...
  const handleOrderPhotoUpload = async (order) => {
    const currentPhotos = photoMap[String(order.id)] || [];
    const remainingSlots = MAX_ORDER_PHOTOS - currentPhotos.length;
    if (remainingSlots <= 0) {
      alert(`작품 사진은 주문 1건당 최대 ${MAX_ORDER_PHOTOS}장까지 등록할 수 있습니다.`);
      return;
    }
    ...
    input.onchange = async (e) => {
      ...
      if (files.length === 0) { cleanupInput(); return; }
      ...
      try {
        for (let i = 0; i < filesToUpload.length; i++) {
          ...
          try {
            ...
            const path = `${order.id}/${Date.now()}_${i}.jpg`;
            ...
            const { error: dbError } = await supabase.from('order_photos').insert([payload]);
            if (dbError) {
        ...
        let msg = '';
        if (successCount > 0) msg += `작품 사진 ${successCount}장이 저장되었습니다. `;
        if (skippedCount > 0) msg += `최대 ${MAX_ORDER_PHOTOS}장 제한으로 ${skippedCount}장은 등록되지 않았습니다. `;
        if (successCount === 0 && lastErrorMessage) msg += `업로드 실패: ${lastErrorMessage}`;
  ...
  // photo: order_photos 테이블의 개별 행(row) 객체 (id, order_id, storage_path 포함)
  const handleOrderPhotoDelete = async (photo) => {
    if (!photo) return;
    if (!window.confirm('이 작품 사진을 삭제하시겠습니까?')) return;
    try {
  ...
  const submitNewOrder = async (shouldPrint) => {
    ...
    const pickupDatetime = newOrder.is_delivery
      ? `${newOrder.delivery_date}T${newOrder.delivery_time}:00`
      : `${newOrder.pickup_date}T${newOrder.pickup_time}:00`;
  ...
  const handleCreateOnsiteOrder = async (e) => {
    ...
    const { error } = await supabase.from('orders').insert([{
      ...
      delivery_date: onsiteOrder.is_delivery ? onsiteOrder.delivery_date : null,
  ...
  const dashboardStats = useMemo(() => {
    ...
    const filtered = validOrders.filter(o => {
      ...
      if (dashboardPeriod === 'year') return d.slice(0, 4) === now.date.slice(0, 4);
      return true;
    });
  ...
  const handleUpdateOrder = async (e) => {
    e.preventDefault();
    const isOnsite = editingOrder.order_type === '현장판매';
    ...
    let customerIdToLink = editingOrder.customer_id || null;
    const trimmedName = (editingOrder.customer_name || '').trim();
    const trimmedPhone = (editingOrder.phone || '').trim();

    if (editingOrder.customer_id) {
      // 기존에 연결된 고객이 있으면 정보만 갱신
      await supabase.from('customers').update({
        name: editingOrder.customer_name,
        phone: editingOrder.phone
      }).eq('id', editingOrder.customer_id);
    } else if (isOnsite && trimmedName && trimmedPhone) {
      // 현장판매 수정 중 고객정보를 새로 입력한 경우: 매칭/신규등록 후 연결
      const { data: custByPhone } = await supabase
        .from('customers')
        .select('id, name')
        .eq('phone', trimmedPhone)
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 flex flex-col lg:flex-row pb-12 lg:pb-0">
      ...
      <aside className="bg-slate-50 border-b lg:border-b-0 lg:border-r border-slate-200 w-full lg:w-28 p-1 lg:p-2 flex flex-col shrink-0 shadow-xs">
        <div className="hidden lg:flex items-center justify-between text-rose-500 font-extrabold text-sm mb-1 px-1">
          <span className="flex items-center gap-1">📌 메뉴</span>
        </div>

        <nav className="flex lg:flex-col items-center lg:items-stretch justify-between lg:justify-start gap-0.5 lg:gap-1 w-full text-xs py-0.5 lg:py-0">
          {menuList.map(menu => (
            <label
              key={menu.id}
              onClick={() => handleMenuChange(menu.id)}
              className={`flex flex-col lg:flex-row items-center justify-center lg:justify-start gap-0 lg:gap-1 px-0.5 lg:px-2 py-1 lg:py-1.5 rounded-lg cursor-pointer whitespace-nowrap transition-all flex-1 lg:flex-none text-center ${
                activeMenu === menu.id
            ...
            >
              <input
                ...
                onChange={() => {}}
                className="w-3 h-3 accent-rose-500 cursor-pointer hidden lg:inline"
              />
              <span className="text-sm leading-none lg:hidden">{menu.icon}</span>
              <span className="text-[9px] leading-tight font-bold lg:hidden">{menu.text}</span>
              <span className="hidden lg:inline text-xs font-bold">{menu.icon} {menu.text}</span>
            </label>
          ))}
        </nav>
      </aside>

      <main className="flex-1 p-2 lg:p-5 max-w-[1600px] mx-auto w-full">
        {photoViewer && viewerPhoto && (
          <div
            ...
            >
              ...
              >
                {viewerPhotos.length > 1 && (
                  <button
                    onClick={() => setPhotoViewerIndex(i => (i - 1 + viewerPhotos.length) % viewerPhotos.length)}
                    className="absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-white/90 hover:bg-white border border-slate-300 text-black text-sm font-bold shadow-sm cursor-pointer"
                    aria-label="이전 사진"
                    style={{ zIndex: 1 }}
                  >
                    ‹
                ...
                {viewerPhotos.length > 1 && (
                  <button
                    onClick={() => setPhotoViewerIndex(i => (i + 1) % viewerPhotos.length)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-white/90 hover:bg-white border border-slate-300 text-black text-sm font-bold shadow-sm cursor-pointer"
                    aria-label="다음 사진"
                    style={{ zIndex: 1 }}
                  >
                    ›
              ...
              {viewerPhotos.length > 1 && (
                <div className="flex items-center justify-center gap-1.5 py-1.5 border-t border-slate-200 bg-white" style={{ flexShrink: 0 }}>
                  {viewerPhotos.map((_, idx) => (
                    <button
                      key={idx}
                      onClick={() => setPhotoViewerIndex(idx)}
                      className={`w-2 h-2 rounded-full cursor-pointer ${idx === photoViewerIndex ? 'bg-rose-500' : 'bg-slate-300 hover:bg-slate-400'}`}
              ...
              <div
                ...
                style={{ flexShrink: 0 }}
              >
                {viewerPhoto.original_name || '-'} · {viewerPhoto.size_bytes ? `${Math.round(viewerPhoto.size_bytes / 1024)}KB` : ''}
              </div>
            </div>
          </div>
        )}
            ...
            <div className="bg-white rounded-2xl p-4 md:p-6 w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-xl border border-slate-200">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-base md:text-lg font-bold text-slate-900">
                  {editingOrder.order_type === '현장판매' ? '✏️ 현장판매 수정' : '✏️ 주문 수정'}
                </h3>
              ...
              {editingOrder.order_type === '현장판매' ? (
                <form onSubmit={handleUpdateOrder} className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <label className="flex items-center gap-1.5 px-2 py-2 border border-slate-300 rounded-xl bg-white cursor-pointer whitespace-nowrap h-fit self-end">
                      <input
                        type="checkbox"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      <label className="text-[11px] font-bold text-slate-700">휴대폰 번호 (선택)</label>
                      <input
                        type="text"
                        inputMode="numeric"
        ...
        {isCalendarOrderModalOpen && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-3 z-50">
            <div className="bg-white rounded-2xl p-4 md:p-6 w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-xl border border-slate-200">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-base md:text-lg font-bold text-slate-900">➕ 신규 주문 입력 ({newOrder.is_delivery ? newOrder.delivery_date : newOrder.pickup_date})</h3>
              ...
              <form onSubmit={handleCreateOrder} className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[11px] font-bold text-slate-700">{newOrder.is_delivery ? '배송 날짜 *' : '픽업 날짜 *'}</label>
                    <input
                      type="date"
                      value={newOrder.is_delivery ? (newOrder.delivery_date || '') : newOrder.pickup_date}
                      onChange={e => newOrder.is_delivery
                        ? setNewOrder({ ...newOrder, delivery_date: e.target.value })
                        : setNewOrder({ ...newOrder, pickup_date: e.target.value })}
                      className="w-full p-2 border border-slate-300 rounded-xl text-xs bg-white text-slate-900 font-medium"
                    />
                  </div>
                  <div>
                    <label className="text-[11px] font-bold text-slate-700">{newOrder.is_delivery ? '배송 시간 *' : '픽업 시간 *'}</label>
                    <TimePickerCustom
                      value={newOrder.is_delivery ? (newOrder.delivery_time || '') : newOrder.pickup_time}
                      onChange={val => newOrder.is_delivery
                        ? setNewOrder({ ...newOrder, delivery_time: val })
                        : setNewOrder({ ...newOrder, pickup_time: val })}
                    />
                  </div>
                </div>

                <div className="gap-1.5" style={{ display: 'flex', alignItems: 'flex-end' }}>
                  ...
                  <label className="flex items-center gap-1 px-1.5 py-2 border border-slate-300 rounded-xl bg-white cursor-pointer whitespace-nowrap" style={{ flexShrink: 0 }}>
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="text-[10px] font-bold text-slate-700">💬알림톡</span>
                  </label>
                  <label className="flex items-center gap-1 px-1.5 py-2 border border-slate-300 rounded-xl bg-white cursor-pointer whitespace-nowrap" style={{ flexShrink: 0 }}>
                    <input
                      type="checkbox"
                      ...
                      className="w-4 h-4 accent-sky-500 cursor-pointer"
                    />
                    <span className="text-[10px] font-bold text-slate-700">🚚배송</span>
                  </label>
                </div>
          ...
          <div className="max-w-2xl mx-auto bg-white p-3 md:p-8 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="flex gap-1.5 mb-4 md:mb-6 border-b border-slate-200 pb-3">
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-xs md:text-sm font-bold cursor-pointer border-2 transition-colors ${
            ...
            <form onSubmit={handleCreateOrder} className="space-y-3 md:space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 md:gap-4">
                <div>
                  <label className="text-[11px] md:text-xs font-bold text-black">{newOrder.is_delivery ? '배송 날짜 *' : '픽업 날짜 *'}</label>
                  <input
                    type="date"
                    value={newOrder.is_delivery ? (newOrder.delivery_date || '') : newOrder.pickup_date}
                    onChange={e => newOrder.is_delivery
                ...
                <div>
                  <label className="text-[11px] md:text-xs font-bold text-black">{newOrder.is_delivery ? '배송 시간 *' : '픽업 시간 *'}</label>
                  <TimePickerCustom
                    value={newOrder.is_delivery ? (newOrder.delivery_time || '') : newOrder.pickup_time}
                    onChange={val => newOrder.is_delivery
                      ? setNewOrder({ ...newOrder, delivery_time: val })
                    ...
                    onChange={e => {
                      if (e.target.checked) {
                        ...
                        setOnsiteOrder({
                          ...onsiteOrder,
                          is_delivery: true,
                          delivery_date: onsiteOrder.delivery_date || getKoreaNowFormatted().date,
                          delivery_time: onsiteOrder.delivery_time || nextQ.time
                        });
                      } else {
                        setOnsiteOrder({ ...onsiteOrder, is_delivery: false, delivery_date: '', delivery_time: '' });
                      }
                    }}
                    className="w-4 h-4 accent-sky-500 cursor-pointer"
                  />
              ...
              {onsiteOrder.is_delivery && (
                <div className="grid grid-cols-2 gap-2 md:gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      value={onsiteOrder.delivery_date || ''}
                      onChange={e => setOnsiteOrder({ ...onsiteOrder, delivery_date: e.target.value })}
                  ...
                  <div>
                    ...
                    <TimePickerCustom
                      ...
                      onChange={val => setOnsiteOrder({ ...onsiteOrder, delivery_time: val })}
                    />
                  </div>
                </div>
              )}
                        ...
                        >
                          <span
                            ...
                            style={{
                              ...
                              backgroundColor: isDeliveryRow ? 'transparent' : (isOnsite ? 'transparent' : '#a5b4fc'),
                              color: undefined
                            }}
        ...
        {activeMenu === 'customers' && (
          <CustomerTab
            ...
            setCustomerNameSort={setCustomerNameSort}
            handleToggleSelectAllCustomers={handleToggleSelectAllCustomers}
            getCustomerOrderCount={getCustomerOrderCount}
            getCustomerNameColor={getCustomerNameColor}
            setCustomerHistoryModal={setCustomerHistoryModal}
            getCustomerPickupDate={getCustomerPickupDate}
            startEditCustomer={startEditCustomer}
            handleToggleSelectCustomer={handleToggleSelectCustomer}
          />
        )}
        ...
        {orderInfoModal && (() => {
          ...
          return (
            <div
              ...
              style={{ zIndex: 9999 }}
              onClick={() => setOrderInfoModal(null)}
            >
              <div
                className="bg-white rounded-2xl p-5 md:p-6 max-w-sm w-full shadow-2xl border border-slate-200"
                onClick={e => e.stopPropagation()}
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button onClick={() => setOrderInfoModal(null)} className="text-slate-400 hover:text-slate-600 text-lg font-bold cursor-pointer">✕</button>
                </div>

                <div className="space-y-2.5 text-sm">
                  <div className="flex justify-between border-b border-slate-100 pb-2">
                    <span className="text-slate-500">번호</span>
                    <span className="font-bold text-slate-900">#{o.id}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-100 pb-2">
                    <span className="text-slate-500">성명</span>
                    <span className="font-extrabold text-slate-900 text-base">{o.customers?.name || '-'}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-100 pb-2">
                    <span className="text-slate-500">연락처</span>
                    <span className="font-extrabold text-slate-900 text-base">{o.customers?.phone || '-'}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-100 pb-2">
                  ...
                  <div className="flex justify-between border-b border-slate-100 pb-2">
                    ...
                    <span className="font-bold text-slate-900">{Number(o.amount || 0).toLocaleString()}원</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-100 pb-2">
                    <span className="text-slate-500">결제수단</span>
                    <span className="font-bold text-slate-900">{o.payment_method || '-'}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-100 pb-2">
                    <span className="text-slate-500">픽업/배송</span>
                    <span className="font-bold text-slate-900">{formatPickupWithDay(o.pickup_datetime)}</span>
                  </div>
                  {o.created_at && (
                    <div className="flex justify-between border-b border-slate-100 pb-2">
                      <span className="text-slate-500">접수일시</span>
                      <span className="font-bold text-slate-900">{formatShortDateTime(o.created_at)}</span>
                    </div>
                  )}
                  <div>
                    <span className="text-slate-500">요청사항</span>
                    <p className="mt-1 text-slate-800 leading-relaxed">{o.memo || '없음'}</p>
                  </div>
                </div>

                <button
                  onClick={() => setOrderInfoModal(null)}
                  className="w-full mt-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold border border-slate-300 cursor-pointer"
                >
                  닫기
                </button>
              </div>
                    ...
                    return (
                      <div key={log.id} className="text-xs">
                        ...
                        <button
                          ...
                          onClick={() => relatedOrder ? setOrderInfoModal(relatedOrder) : alert('연결된 주문 정보를 찾을 수 없습니다. (삭제되었거나 오래된 기록일 수 있습니다)')}
                        >
                          <span
          ...
          <div className="bg-white p-4 md:p-8 rounded-2xl border border-slate-200 shadow-sm max-w-xl mx-auto space-y-6">
            ...
            <div className="p-4 border border-slate-200 rounded-xl bg-slate-50 space-y-2">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-xs md:text-sm text-slate-800">📦 작품 사진 저장 용량 (Supabase 무료 플랜)</h3>
                <span className={`text-xs font-extrabold ${photoStorageUsedPercent >= 90 ? 'text-red-600' : photoStorageUsedPercent >= 70 ? 'text-amber-600' : 'text-emerald-600'}`}>
                  {photoStorageUsedMB.toFixed(1)}MB / {SUPABASE_STORAGE_LIMIT_MB}MB
                </span>
              </div>
              <div className="w-full h-2.5 rounded-full bg-slate-200 overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${photoStorageUsedPercent >= 90 ? 'bg-red-500' : photoStorageUsedPercent >= 70 ? 'bg-amber-500' : 'bg-emerald-500'}`}
                  style={{ width: `${photoStorageUsedPercent}%` }}
                />
              </div>
              <p className="text-[11px] text-slate-500">
                등록된 작품 사진 {photoTotalCount}장 (주문당 최대 {MAX_ORDER_PHOTOS}장) 기준 실시간 합산입니다. 90% 이상이면 새 사진 업로드가 거부될 수 있으니 용량 확보나 요금제 업그레이드를 검토하세요.
              </p>
            </div>
        ...
        {activeMenu === 'ribbon' && (
          <RibbonEditor
            ribbonText={ribbonText} setRibbonText={setRibbonText}
            ribbonFontFamily={ribbonFontFamily} setRibbonFontFamily={setRibbonFontFamily}
            ribbonFontSize={ribbonFontSize} setRibbonFontSize={setRibbonFontSize}
            ribbonLetterSpacing={ribbonLetterSpacing} setRibbonLetterSpacing={setRibbonLetterSpacing}
            ribbonWordSpacingEm={ribbonWordSpacingEm} setRibbonWordSpacingEm={setRibbonWordSpacingEm}
            ribbonLineGap={ribbonLineGap} setRibbonLineGap={setRibbonLineGap}
            ribbonPaperWidth={ribbonPaperWidth} setRibbonPaperWidth={setRibbonPaperWidth}
            ribbonMarginLeft={ribbonMarginLeft} setRibbonMarginLeft={setRibbonMarginLeft}
            ribbonMarginRight={ribbonMarginRight} setRibbonMarginRight={setRibbonMarginRight}
            ribbonMarginTop={ribbonMarginTop} setRibbonMarginTop={setRibbonMarginTop}
            ribbonMarginSync={ribbonMarginSync} setRibbonMarginSync={setRibbonMarginSync}
            ribbonBold={ribbonBold} setRibbonBold={setRibbonBold}
            ribbonShowGuide={ribbonShowGuide} setRibbonShowGuide={setRibbonShowGuide}
            ribbonCopies={ribbonCopies} setRibbonCopies={setRibbonCopies}
            ribbonTemplates={ribbonTemplates}
            selectedRibbonTemplateId={selectedRibbonTemplateId} setSelectedRibbonTemplateId={setSelectedRibbonTemplateId}
            ribbonTemplateLoading={ribbonTemplateLoading} setRibbonTemplateLoading={setRibbonTemplateLoading}
            fetchRibbonTemplates={fetchRibbonTemplates}
            applyRibbonTemplate={applyRibbonTemplate}
          />
        )}
      </main>
    </div>
